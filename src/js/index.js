...
const plugin = _ => {
    ...
    addFilter('CREATE_VIEW', viewAPI => {
        ...
        const didLoadItem = ({ root, props }) => {
            ...
            document.addEventListener('FilePond:checkimage', ({ fileId, e }) => {
                if (fileId === id) {
                    return;
                }
                root.ref.handleUnselect(e);
            ...
            if (canShowImagePreview) {
                ...
                // select item classname
                buttonCheckView.element.classList.add('filepond--action-check-item');
                buttonCheckView.element.dataset.align = position;
                buttonCheckView.element.title = query('GET_SELECT_IMAGE_BUTTON_TITLE');
                buttonCheckView.on('click', root.ref.handleUnselect);

                root.ref.buttonCheckItem = view.appendChildView(buttonCheckView);
            }
        };
